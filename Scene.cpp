...
void Scene::Draw(Shader shader, float radians)
{
	...
	for (GLuint modelCounter = 0; modelCounter < this->models.size(); modelCounter++)
	{
		...
		modelProperties = glm::translate(modelProperties, thisModel.translation);
		modelProperties = glm::rotate(modelProperties, glm::radians(radians), thisModel.rotate);
...
void Scene::loadModel(string path, GLuint &ID)
{
	...
	thisModel.rotate = glm::vec3(0.0, 0.0, 1.0);

...
{
public:
	Scene(const GLchar* path, GLuint &ID)
	{
		this->loadModel(path, ID);
	}
	~Scene();
	void Draw(Shader shader, float radians);
	...
	void setModel(GLuint ID, bool toDraw, glm::vec3 trans, glm::vec3 scal, glm::vec3 rota)
	{
		setModel(ID, toDraw);
		models[ID].translation = trans;
		models[ID].scale = scal;
		models[ID].rotate = rota;
	}
